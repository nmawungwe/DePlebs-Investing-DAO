...
interface IFakeNFTMarketplace {
    /// @dev nftPurcahsePrice() reads the value of the public uint256 variable `nftPurchasePrice`
    function nftPurchasePrice() external view returns (uint256);

    /// @dev available returns whether or not the given _tokenId has already been purchased 
    /// @return Returns a boolean value - true if avalaible, false if not 
    ...
    /// @param _tokenId the fake NFT tokenId to sell back
...
contract DePlebsDAO is IERC721Receiver {
    ...
    struct Member {
        uint256 joinedAt;
        // array of tokenIds for dePlebs NFT that are locked up by the DAO member
    ...
    public override returns (bytes4) {
        require(deplebsNft.ownerOf(tokenId) == address(this), "MALICIOUS");
    ...
    function quit() external memberOnly {
        ...
        require(
            ...
            "MIN_MEMBERSHIP_PERIOD_NOT_EXCEEDED"
        );
